...
.PHONY: insight
insight: $(patsubst src/%,build/insight/%,${source_files} ${header_files})

# @todo Handle dependencies in insight files (probably base on the '*.d' files generated by 'g++ -MMD')
build/insight/%: src/%
	@${echo} "Insight: insight $<"
	@mkdir -p ${@D}
	@insights --show-all-callexpr-template-parameters --show-all-implicit-casts $< -- $$(pkg-config cairomm-1.16 libavutil libavcodec --cflags) -std=c++20 >$@.tmp
	@mv $@.tmp $@
